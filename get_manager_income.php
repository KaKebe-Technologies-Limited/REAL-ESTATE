...
if (!isset($_SESSION['user_id']) || $_SESSION['user_type'] !== 'manager') {
    echo json_encode([
        'success' => false,
        'message' => 'Unauthorized access. Please login as a manager.'
    ]);
    exit;
}
...
try {
    // Calculate rental income for properties managed by this manager
    ...
    // Calculate sales income for properties managed by this manager
    ...
    $distribution_query = "SELECT
                            (SELECT COUNT(*) FROM rental_property WHERE manager_id = ?) as rental_count,
                            (SELECT COUNT(*) FROM sales_property WHERE manager_id = ?) as sales_count";
